...
class Puzzle(object):
    ...
    def heuristic_function_caleb(state, goal_state): #estimated cost to get from the node to the goal
        ...
        #Do a swap
        def search(self, matrix, value): # return a pair of i j values
            length = len(matrix)
            for i in range(0, length):
                for j in range(0, length):
                    if matrix[i][j] == value:
    ...
    class Experimental_evaluation():
        ...
        def conclude(self):
            if self.number_of_nodes != 0:
                ...
                return [self.average_f, self.number_of_nodes]
            else:
                return [-1,-1]
    ...
    class Node():
        def __init__(self, state, g):
            self.state = state
            self.g = g
            self.h = Puzzle.heuristic_function_manhatten(state, goal_state) #use of heuristic here
...
if __name__ == "__main__":
    ...
    sys.argv.append(input_file + "output_4.txt")
    ...
    heu = puzzle.evaluate_heuristic()
    if heu == [-1,-1]:
        print("UNSOLVABLE")
    else :
        print(heu) # returns average f and number of nodes explored
